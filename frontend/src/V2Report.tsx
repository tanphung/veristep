...
export function V2Report({deal}:{deal:V2Deal}){
  ...
  return <>
    <section className="v2-decision" aria-labelledby="decision-title">
      ...
      {deal.status==="INCONCLUSIVE"&&<div className="review-state-note" role="note"><h3>Review inconclusive</h3><p>GenLayer could not conclusively assess one or more obligations from the available evidence. Uncertainty alone is not treated as a violation.</p><p>Determinate role outcomes and each agent’s recorded entitlements are preserved. Settlement activation is withheld during the frozen adjudication window.</p>{deal.adjudication_deadline!==undefined&&<p>Adjudication deadline: <time dateTime={new Date(deal.adjudication_deadline*1000).toISOString()}>{new Date(deal.adjudication_deadline*1000).toLocaleString()}</time>.</p>}<p>After the deadline, a successful timeout transition creates settlement legs from this report. Expiry alone does not activate or dispatch transfers. <a href="#docs?section=inconclusive">How inconclusive settlement works</a></p></div>}
      {(["A","B"] as const).map(role=><article className="agent-result" aria-label={`Agent ${role} result`} key={role}><header><span>Agent {role}</span><em className={`verdict ${statusClass(report.decision.stages[role].outcome)}`}>{title(report.decision.stages[role].outcome)}</em></header><div className="agent-score"><strong>{score(report.score[role])}</strong><span>{report.score[role]===null?"Task requirements could not be fully assessed":"Task requirements met"}</span></div>{report.decision.stages[role].outcome==="UNASSESSABLE"&&<p className="section-helper">Uncertainty alone is not a violation. This role has no fee payout or violation penalty; its fee returns to the client and its deposited bond is returned.</p>}<details className="role-entitlements"><summary>Recorded entitlements · Agent {role}</summary><dl>{(["PAYOUT","REFUND","BOND_RETURN"] as const).map(kind=><div key={kind}><dt>{kind==="PAYOUT"?"Agent fee":kind==="REFUND"?"Client refund":"Agent bond return"}</dt><dd>{report.decision.stages[role].entitlements[kind]} attoGEN</dd></div>)}</dl><p>Contract-recorded allocations; not proof of dispatched or received payment.</p></details></article>)}
    ...
    <section className="v2-section" id="settlement"><header><div><span className="eyebrow">RECEIPT-BOUND SETTLEMENT</span><h2>Outcome and payment remain separate proofs</h2>{deal.settlement_legs.some(leg=>leg.state==="ELIGIBLE")&&<p className="section-helper">Allocations are ready. Each eligible leg still needs an explicit native-transfer dispatch; settlement pending does not mean paid.</p>}{deal.settlement_legs.some(leg=>leg.state==="DISPATCHED_UNVERIFIED")&&<p className="section-helper">Native transfer dispatched; contract-side receipt verification is unavailable on Studio Next.</p>}</div><Landmark/></header><div className="receipt-list">{deal.settlement_legs.length?deal.settlement_legs.map(leg=>{const proof=activityForSettlementLeg(deal.deal_id,leg.id);return <article key={leg.id}><span className={`receipt-state ${leg.state.toLowerCase()}`}>{settlementStateLabel(leg.state)}</span><div><strong>{title(leg.kind)} · Worker {leg.role}</strong><small>Recipient {short(leg.recipient)}</small>{proof&&<InlineTransactionProof item={proof}/>}</div><div className="receipt-amount"><strong>{leg.amount}</strong><small>attoGEN</small></div><details className="receipt-technical"><summary>Technical details</summary><div><code>{leg.state}</code><code title={leg.receipt_id}>{short(leg.receipt_id)}</code></div></details></article>}):<p>{deal.status==="INCONCLUSIVE"?"The report is recorded; settlement legs are not activated yet. Each role’s entitlements remain preserved until the timeout transition can create its legs.":"No settlement legs are recorded in this finalized state."}</p>}</div></section>
  </>;
}

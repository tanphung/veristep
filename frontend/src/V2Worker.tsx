...
export function V2Worker({deal,account}:{deal:V2Deal;account?:Address}){
  ...
  const serviceState=state?.stage.replaceAll("_"," ")??(!workerUrl?"SERVICE UNAVAILABLE":healthError?"SERVICE UNAVAILABLE":!health?"CHECKING SERVICE":!health.ready?"SERVICE BLOCKED":!healthMatchesDeal?"CONFIGURATION MISMATCH":!eligible?"NOT ELIGIBLE":!client?"WALLET REQUIRED":"SERVICE READY");
  const detail=state?.detail||(!workerUrl?"The hosted service URL is not configured.":healthError||(!health?"Checking the hosted service…":!health.ready?"The hosted service reports an unmet server-side dependency. Reviewer credentials are never required.":!healthMatchesDeal?"The hosted worker wallets do not match this deal's frozen participants.":!eligible?`This record is ${deal.status.replaceAll("_"," ")} and cannot start a new hosted delivery.`:!client?"Connect the frozen client wallet to authorize this exact deal.":"Server-side AI and evidence publishing are ready. One wallet signature authorizes this exact deal only."));
  return <section className="v2-worker"><header><div className="worker-icon"><Bot/></div><div><span className="eyebrow">HOSTED AGENT HANDOFF</span><h2>Agent A extracts · Agent B reports</h2><p>The agents publish immutable work. GenLayer validators—not this service—verify evidence and decide settlement.</p></div><span className={`worker-state ${state?.state.toLowerCase()??(serviceReady?"ready":"locked")}`}>{serviceState}</span></header>{error&&<p className="error">{error}</p>}<div className="worker-controls">{!runId&&<button className="primary" disabled={!client||!eligible||busy||!serviceReady} onClick={()=>void command("start")}><Play/> Start A/B delivery</button>}{runId&&<button onClick={()=>void load()} disabled={busy}><RefreshCw/> Refresh checkpoint</button>}{state?.state==="ERROR"&&<button className="primary" onClick={()=>void command("resume")} disabled={!client||busy}><Play/> Resume safely</button>}{state&&["ACTIVE","ERROR"].includes(state.state)&&<button onClick={()=>void command("cancel")} disabled={!client||busy}><PauseCircle/> Stop workflow</button>}<small>{detail}</small></div></section>;
}

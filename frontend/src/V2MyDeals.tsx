...
export function V2MyDeals({account,ids,selected}:{account?:string;ids:string[];selected:string}){
  ...
  useEffect(()=>{
    if(!account)return;
    let cancelled=false;
    setResult({account,deals:[],loading:true,failed:false});
    void (async()=>{
  ...
  return <section className="my-deals" aria-label="My deals"><strong>My deals</strong>
    {!account?<p className="meta">Connect your wallet to see deals you created.</p>:<>
      {(!current||current.loading)&&<p className="meta" role="status">Finding your deals…</p>}
      {current&&!current.loading&&<>
        {current.failed&&<p className="meta" role="status">Some deals could not be checked. <button onClick={()=>setRetry(value=>value+1)}>Retry my deals</button></p>}
        {!current.failed&&!current.deals.length&&<p className="meta">No deals yet for this wallet. Select New Deal to create one.</p>}

...
let verifiedProofsPending:Promise<Record<string,V2Deal>>|undefined;
function loadVerifiedProofs(fresh:boolean){
  if(!fresh&&verifiedProofs)return Promise.resolve(verifiedProofs);
  if(verifiedProofsPending)return verifiedProofsPending;
  const request=Promise.all(canonicalReleaseProofs.map(item=>readFinalizedWithRetry(()=>readV2Deal(item.id,fresh)))).then(deals=>{
    verifiedProofs=Object.fromEntries(deals.map(deal=>[deal.deal_id,deal]));
...
export function V2Compare({ids}:{ids:string[]}){
  ...
  const [proofs,setProofs]=useState<Record<string,V2Deal>>(verifiedProofs??{}),[proofError,setProofError]=useState(""),[proofLoading,setProofLoading]=useState(!verifiedProofs);
  ...
  useEffect(()=>{let live=true;if(verifiedProofs&&retry===0){setProofs(verifiedProofs);setProofLoading(false);return;}setProofLoading(true);setProofError("");void loadVerifiedProofs(retry>0).then(deals=>{if(live)setProofs(deals);}).catch(cause=>{if(live)setProofError(finalizedReadError(cause,"Could not load the verified examples. Please try again."));}).finally(()=>{if(live)setProofLoading(false);});return()=>{live=false;};},[retry]);
